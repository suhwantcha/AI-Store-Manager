...
SETTLEMENT_FILE_PATH = os.path.join(DATA_DIR, "settlement.json")
SELLER_INFO_FILE_PATH = os.path.join(DATA_DIR, "seller_info.json") # 추가

def get_db_connection():
    """PostgreSQL 연결 객체를 반환합니다."""
    try:
        # psycopg2 accepts the connection URI
...
def seed_database():
    ...
    # 1.5 SQLAlchemy 테이블 생성 (StoreSettings, AICache)
    try:
        from backend.database.session import engine
        from backend.models.orm import Base
        Base.metadata.create_all(bind=engine)
        print("✅ SQLAlchemy 테이블 생성이 완료되었습니다.")
    except Exception as e:
        print(f"⚠️ SQLAlchemy 테이블 생성 중 오류 발생: {e}")
        conn.close()
        return
    ...
    data_to_load = {
        ...
        "settlement": (SETTLEMENT_FILE_PATH, "settlement"),
        "seller_info": (SELLER_INFO_FILE_PATH, "store_settings") # 추가
    }
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Store settings (단일 행 삽입)
            seller_info = all_data["seller_info"]
            if seller_info:
                store_info = seller_info.get("storeInfo", {})
                delivery = seller_info.get("deliveryPolicy", {})
                claim = seller_info.get("claimPolicy", {})
                cs = seller_info.get("csInfo", {})
                logistics = seller_info.get("logisticsInfo", {})
                
                cur.execute("""
                    INSERT INTO store_settings (
                        store_name, ceo_name, delivery_base_fee, free_shipping_threshold,
                        same_day_cutoff, return_address, return_fee, exchange_fee,
                        cs_phone, cs_hours, logistics_company, outbound_location, settings_json
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (
                    store_info.get("storeName"), store_info.get("ceoName"),
                    delivery.get("baseFee"), delivery.get("freeConditionalAmount"), delivery.get("thisDayDispatchCutoff"),
                    claim.get("returnAddress"), claim.get("returnFee"), claim.get("exchangeFee"),
                    cs.get("csNumber"), cs.get("csOperatingHours"),
                    logistics.get("mainDeliveryCompany"), logistics.get("outboundLocation"),
                    json.dumps(seller_info, ensure_ascii=False)
                ))
